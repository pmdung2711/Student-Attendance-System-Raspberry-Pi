...
ap.add_argument("-e", "--embeddings", default = "Models/embeddings_update.pickle")
...
def Extract_embeddings(imagePaths):
    total = 0
...
def update(path, id):
    ...
    try:
        old_data = pickle.loads(open(args["embeddings"], "rb").read())
    ...
    knownEmbeddings = Extract_embeddings(imagePaths)

    for item in knownEmbeddings:
        new_embeddings.append(item)
        new_names.append(str(id))

    data1 = {"embeddings": new_embeddings, "names": new_names}

    with open(args["embeddings"], 'wb') as fp:
        pickle.dump(data1, fp)

    num = open("Models/number.txt", "a")
    num.write(str(id))
    num.write("\n")
    num.close()
